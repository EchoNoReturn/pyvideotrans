    ...
    def _item_task(self, data_item: dict = None):
        ...
        try:
            ...
            if self.api_url.endswith(":9880"):
                data = {"text": text, "speed": 1 + rate, "new": 0, "streaming": 0}
                ...
                if role == "clone":
                    ...
                    data["speaker"] = (
                        tmp
                        if isinstance(tmp, str) or "reference_audio" not in tmp
                        else tmp["reference_audio"]
                    )
                else:
                    data["speaker"] = "中文女"

                if data["speaker"] not in [
                    "中文男",
                    "中文女",
                    "英文男",
                    "英文女",
                ...
                response = requests.post(
                    f"{self.api_url}", json=data, proxies=self.proxies, timeout=3600
                )
            else:
