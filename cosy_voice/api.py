...
def get_params(req):
    ...
    # 原始字符串
    params["text"] = (
        req.args.get("text", "").strip() or req.form.get("text", "").strip()
    )

    # 字符串语言代码
    params["lang"] = (
        req.args.get("lang", "").strip().lower()
        or req.form.get("lang", "").strip().lower()
    )
    # 兼容 ja语言代码
    if params["lang"] == "ja":
        params["lang"] = "jp"
    ...
    # 要克隆的音色文件
    params["reference_audio"] = req.args.get("reference_audio", None) or req.form.get(
        "reference_audio", None
    )
    ...
    # 音色文件对应文本
    params["reference_text"] = req.args.get(
        "reference_text", ""
    ).strip() or req.form.get("reference_text", "")
...
def batch(tts_type, outname, params):
    ...
    if tts_type != "tts":
        if not params["reference_audio"] or not os.path.exists(
            f"{root_dir}/{params['reference_audio']}"
        ):
    ...
    if tts_type == "tts":
        ...
        for i, j in enumerate(
            sft_model.inference_sft(
                text, params["role"], stream=False, speed=params["speed"]
            )
        ):
        ...
        print(text)
        for i, j in enumerate(
            tts_model.inference_cross_lingual(
...
def tts():
    ...
    if not params["text"]:
        return make_response(
            jsonify({"code": 1, "msg": "缺少待合成的文本"}), 500
        )  # 设置状态码为500
    try:
        # 仅文字合成语音
...
def clone_eq():
    try:
        ...
        if not params["reference_text"]:
            return make_response(
                jsonify({"code": 6, "msg": "同语言克隆必须传递引用文本"}), 500
            )  # 设置状态码为500
